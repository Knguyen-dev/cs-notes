...
* actually affect the internal behavior of those syscalls. If read-buffer isn't empty, both blocking/non-blcoking will return data. However when empty, reading on a blocking socket 
...
* 
* Even though cur is a `const uint8_t *`, std::string has constructors and the assign fcuntion takes raw character data. We also know that `uint8_t` is just an alias for 
...
// Response::status; just some enums that we can use
enum {
    RES_OK = 0,     
    RES_ERR = 1,    // error
...
static bool try_one_request(Conn *conn) {
    ...
    // Validate that the message's paylaod is within reason.
    uint32_t len = 0;
...
int main() {
    ...
    - **fd_to_connection:** Array of connection objects representing TCP connection sockets. These are defined outside the event loop, which means that a given TCP connection
     ...
     but also booleans indicating our intention with the socket, whether we wnat to read, write, or even close the socket (due to error or if we're actually done processing it).
    ...
    */
    ...
    while (true) {
        ...
        // First is listening socket and rest are connection scokets. 
        poll_args.clear();
